...
void MainWidget::resizeEvent(QResizeEvent* resize_event) {
    ...
    if ((current_widget_stack.size() > 0)) {
        for (auto w : current_widget_stack) {
            QResizeEvent* resize_event_copy = new QResizeEvent(resize_event->size(), resize_event->oldSize());
            QCoreApplication::postEvent(w, resize_event_copy);
        }
    }

    if (text_selection_buttons_) {
        QCoreApplication::postEvent(get_text_selection_buttons(), new QResizeEvent(resize_event->size(), resize_event->oldSize()));
    }
    if (search_buttons_) {
        QCoreApplication::postEvent(get_search_buttons(), new QResizeEvent(resize_event->size(), resize_event->oldSize()));
    }
    if (highlight_buttons_) {
        QCoreApplication::postEvent(get_highlight_buttons(), new QResizeEvent(resize_event->size(), resize_event->oldSize()));
    }
    if (draw_controls_) {
        QCoreApplication::postEvent(get_draw_controls(), new QResizeEvent(resize_event->size(), resize_event->oldSize()));
    }
